<x-app-layout>
    <div class="min-h-screen bg-gray-50">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3 space-x-reverse">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <a href="{{ route('reviews.user') }}" class="mr-1 text-sm font-medium text-gray-700 hover:text-orange-600 md:mr-2">
                                    تقييماتي
                                </a>
                            </div>
                        </li>
                        <li aria-current="page">
                            <div class="flex items-center">
                                ...
                                <span class="mr-1 text-sm font-medium text-gray-500 md:mr-2">تعديل تقييم</span>
                            </div>
                        </li>
                    </ol>
                </nav>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
            <div class="bg-white rounded-2xl shadow-sm border p-6 lg:p-10">
                <div class="mb-8">
                    <h1 class="text-3xl font-bold text-gray-900 mb-4">تعديل تقييم</h1>
                    <div class="flex items-center">
                        <div class="mr-4">
                ...
                <form action="{{ route('reviews.update', $review) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-4">
                            @for ($i = 5; $i >= 1; $i--)
                                <div class="relative">
                                    <input type="radio" name="rating" id="rating-{{ $i }}" value="{{ $i }}" class="peer sr-only" required {{ old('rating', $review->rating) == $i ? 'checked' : '' }}>
                    ...
                    <!-- Review Text -->
                    <div>
                        <label for="review" class="block text-lg font-semibold text-gray-800 mb-3">تقييمك (اختياري)</label>
                        <textarea 
                            id="review" 
                            name="review" 
                            rows="5" 
                    ...
                    @if ($review->images->isNotEmpty())
                        <div>
                            <label class="block text-lg font-semibold text-gray-800 mb-3">الصور الحالية</label>
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                                @foreach ($review->images as $image)
                                    <div class="relative group">
                                        <img src="{{ $image->url }}" class="h-32 w-full object-cover rounded-lg" alt="صورة التقييم">
                                        <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 flex items-center justify-center transition-all rounded-lg">
                                            <div class="opacity-0 group-hover:opacity-100 transition-opacity">
                                                <label class="flex items-center justify-center bg-white rounded-full w-8 h-8 cursor-pointer">
                                                    <input type="checkbox" name="deleted_images[]" value="{{ $image->id }}" class="sr-only">
                            ...
                            <p class="text-sm text-gray-500">حدد الصور التي ترغب في حذفها</p>
                        </div>
                    @endif

                    <!-- Image Upload -->
                    <div>
                        <label class="block text-lg font-semibold text-gray-800 mb-3">أضف صور جديدة (اختياري)</label>
                        ...
                        <div class="flex">
                            ...
                            <div class="mr-3">
                                <p class="text-sm text-yellow-700">
                                    سيتم مراجعة تقييمك من قبل الإدارة مرة أخرى بعد التعديل. يرجى الالتزام بسياسات المنصة.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-4">
                        ...
                        <a href="{{ route('reviews.user') }}" class="px-8 py-3 bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold rounded-lg transition-colors">
                            إلغاء
                        </a>
                    </div>
                </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const imageCheckboxes = document.querySelectorAll('input[name="deleted_images[]"]');
